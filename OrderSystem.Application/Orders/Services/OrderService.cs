...
public class OrderService : IOrderService
{
    ...
    {
        EnsureAuthenticatedUserId(currentUserId);
    ...
    )
    {
        ...
        EnsureUserCanAccessOrder(order, currentUserId, isAdmin);
    ...
    {
        var createdByUserId = isAdmin ? null : EnsureAuthenticatedUserId(currentUserId);
    ...
    public async Task<OrderResponse> RetryPaymentAsync(
        Guid id,
        string currentUserId,
        bool isAdmin,
        CancellationToken cancellationToken
    ...
    private static void EnsureUserCanAccessOrder(Order order, string? currentUserId, bool isAdmin)
    {
        if (isAdmin)
        {
            return;
        ...
        var authenticatedUserId = EnsureAuthenticatedUserId(currentUserId);

        if (order.CreatedByUserId != authenticatedUserId)
        {
            throw new UnauthorizedAccessException("You are not allowed to access this order.");
        }
    }

    private static string EnsureAuthenticatedUserId(string? currentUserId)
    {
